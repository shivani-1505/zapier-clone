...
// OpenModal opens a modal in Slack
func (c *Client) OpenModal(request ModalRequest) error {
	url := "https://slack.com/api/views.open"

	payload, err := json.Marshal(request)
	if err != nil {
		return fmt.Errorf("error marshaling modal request: %w", err)
	}

	req, err := http.NewRequest("POST", url, bytes.NewBuffer(payload))
	if err != nil {
		return fmt.Errorf("error creating modal request: %w", err)
	}
	...
	resp, err := c.HTTPClient.Do(req)
	if err != nil {
		return fmt.Errorf("error sending modal request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		body, _ := ioutil.ReadAll(resp.Body)
		return fmt.Errorf("error opening modal, status: %d, response: %s", resp.StatusCode, string(body))
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&response); err != nil {
		return fmt.Errorf("error decoding modal response: %w", err)
	}
