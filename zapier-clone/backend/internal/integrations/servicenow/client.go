...
// Add this to the ServiceNow Client struct methods

// getFinding gets a finding from ServiceNow by ID
func (c *Client) getFinding(findingID string) (map[string]interface{}, error) {
	resp, err := c.makeRequest("GET", fmt.Sprintf("api/now/table/sn_audit_finding/%s", findingID), nil)
	if err != nil {
		return nil, fmt.Errorf("error getting finding from ServiceNow: %w", err)
	}
	defer resp.Body.Close()
